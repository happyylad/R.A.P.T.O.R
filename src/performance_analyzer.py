...
class PerformanceAnalyzer:
    ...
    def analyze_detection_stats(self):
        ...
        print(f"\n🎯 CONFIDENCE BY CLASS:")
        conf_by_class = df.groupby('class')['confidence'].agg(['mean', 'std', 'count', 'min', 'max'])
        ...
        print(f"   High confidence (>0.8): {len(high_conf)} ({len(high_conf)/total_detections*100:.1f}%)")
        print(f"   Medium confidence (0.6-0.8): {len(medium_conf)} ({len(medium_conf)/total_detections*100:.1f}%)")
        print(f"   Low confidence (<0.6): {len(low_conf)} ({len(low_conf)/total_detections*100:.1f}%)")
        
        # GPS coverage analysis
        with_gps = df.dropna(subset=['gps'] if 'gps' in df.columns else [])
        if len(with_gps) > 0:
            print(f"\n🗺️ GPS COVERAGE:")
            print(f"   Detections with GPS: {len(with_gps)} ({len(with_gps)/total_detections*100:.1f}%)")
            
            # GPS coordinate analysis
            if len(with_gps) > 1:
                gps_data = pd.json_normalize(with_gps['gps'].tolist())
                print(f"   Latitude range: {gps_data['lat'].min():.6f} to {gps_data['lat'].max():.6f}")
                print(f"   Longitude range: {gps_data['lon'].min():.6f} to {gps_data['lon'].max():.6f}")
        ...
        if 'frame' in df.columns:
            ...
            print(f"   Frames with >5 detections: {len(frame_counts[frame_counts > 5])}")
    ...
    def create_visualizations(self, output_dir='output/analysis'):
        ...
        fig.suptitle('🦅 R.A.P.T.O.R Performance Analysis Dashboard', 
        ...
        ax2.hist(df['confidence'], bins=20, color='#00ff41', alpha=0.7, edgecolor='black')
        ax2.set_title('Confidence Score Distribution', fontweight='bold', color='#00ff41')
        ...
        if 'frame' in df.columns:
            ...
            ax4.plot(detections_per_frame.index, detections_per_frame.values, 
            ...
            ax4.fill_between(detections_per_frame.index, detections_per_frame.values, alpha=0.3)
        
        # 5. Confidence vs Class heatmap
        ax5 = plt.subplot(3, 3, 5)
        confidence_bins = pd.cut(df['confidence'], bins=[0, 0.6, 0.8, 1.0], 
        ...
        if 'gps' in df.columns:
            ...
            if len(gps_detections) > 0:
                ...
                scatter = ax6.scatter(gps_data['lon'], gps_data['lat'], 
                                    c=gps_detections['confidence'], cmap='plasma', 
                                    s=60, alpha=0.7, edgecolors='black')
                ax6.set_title('GPS Distribution (Colored by Confidence)', fontweight='bold', color='#00ff41')
...
🗺️ GPS Integration: {'✅ ACTIVE' if 'gps' in df.columns else '❌ DISABLED'}
        ...
        bars = ax9.bar(quality_categories, quality_counts, color=colors, alpha=0.8, edgecolor='black')
        ax9.set_title('Detection Quality Distribution', fontweight='bold', color='#00ff41')
        ax9.set_ylabel('Number of Detections')
        
        # Add value labels on bars
        for bar, count in zip(bars, quality_counts):
            height = bar.get_height()
            ax9.text(bar.get_x() + bar.get_width()/2., height + 0.01*max(quality_counts),
    ...
    def create_detailed_plots(self, output_dir, df):
        ...
        for cls in df['class'].unique():
            class_data = df[df['class'] == cls]
            plt.hist(class_data['confidence'], bins=15, alpha=0.6, label=cls, density=True)
        
        plt.title('Confidence Distribution by Object Class', fontsize=16, fontweight='bold')
        plt.xlabel('Confidence Score')
        plt.ylabel('Density')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(f'{output_dir}/confidence_by_class.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if 'frame' in df.columns:
            ...
            ax1.plot(frame_data['frame'], frame_data['count'], color='#00ff41', linewidth=2)
            ax1.fill_between(frame_data['frame'], frame_data['count'], alpha=0.3, color='#00ff41')
            ax1.set_title('Detection Count Over Time', fontweight='bold')
            ax1.set_ylabel('Detections per Frame')
            ax1.grid(True, alpha=0.3)
            
            # Average confidence over time
            ax2.plot(frame_data['frame'], frame_data['avg_confidence'], color='orange', linewidth=2)
            ax2.set_title('Average Confidence Over Time', fontweight='bold')
            ax2.set_ylabel('Average Confidence')
            ax2.grid(True, alpha=0.3)
            
            # Class diversity over time
            ax3.plot(frame_data['frame'], frame_data['unique_classes'], color='purple', linewidth=2)
    ...
    def tactical_assessment(self):
        ...
        if len(personnel) > 0:
            print(f"   Average Personnel Confidence: {personnel['confidence'].mean():.1%}")
        ...
        print(f"   Military Accuracy Standard (>75%): {'✅ MET' if military_standard else '❌ BELOW'}")
        
        # Real-time capability
        real_time_capable = total_detections > 0  # Simplified check
        print(f"   Real-time Processing: {'✅ CAPABLE' if real_time_capable else '❌ INSUFFICIENT'}")
        
        # Detection diversity
        class_diversity = len(df['class'].unique())
        diverse_detection = class_diversity >= 3
        print(f"   Object Type Diversity: {'✅ ADEQUATE' if diverse_detection else '⚠️ LIMITED'} ({class_diversity} types)")
        ...
        surveillance_score = (avg_confidence * 0.4 + gps_coverage * 0.3 + 
                            (len(personnel) > 0) * 0.3) * 100
        print(f"   Personnel Surveillance: {surveillance_score:.0f}% capability")
        
        # Vehicle tracking
        vehicle_score = (len(vehicles) > 0) * 0.4 + avg_confidence * 0.3 + gps_coverage * 0.3
        vehicle_score = vehicle_score * 100
        print(f"   Vehicle Tracking: {vehicle_score:.0f}% capability")
        
        # Area monitoring
        area_score = (gps_coverage * 0.5 + (class_diversity >= 3) * 0.3 + 
                     (total_detections > 10) * 0.2) * 100
    ...
    def calculate_coverage_area(self, gps_data):
        ...
        lon_km = lon_range * 111 * np.cos(np.radians(gps_data['lat'].mean()))  # Adjust for longitude
...
if __name__ == "__main__":
    ...
    if detection_dir.exists():
        ...
        sample_detections = [
            {'class': 'car', 'confidence': 0.85, 'gps': {'lat': 36.407, 'lon': -105.572}, 'frame': 1},
            {'class': 'person', 'confidence': 0.92, 'gps': {'lat': 36.406, 'lon': -105.571}, 'frame': 5},
            {'class': 'truck', 'confidence': 0.78, 'gps': {'lat': 36.405, 'lon': -105.570}, 'frame': 10}
        ]
